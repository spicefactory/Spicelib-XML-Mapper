...
[Metadata(name="ChildTextNode", types="property")]
/**
 * Represents a Metadata tag that can be used on properties that should map to text nodes wrapped by child elements.
 *
 * @author Jens Halm
 */
public class ChildTextNodeDecorator implements MetadataMapperDecorator {
	...
	public function decorate (builder:MetadataMapperBuilder) : void {
		if (!name) {
			name = builder.namingStrategy.toXmlName(property);
		}
		if (!uri) {
			uri = builder.elementName.uri;
		}
		builder.mapToChildTextNode(property, new QName(uri, name));
	}
